...
<section id="gynecology-dashboard" class="card gyneco-card" data-care-tab-panel="clinical" data-pregnancy-id="{{ ($dashboard && $dashboard['active_pregnancy']) ? $dashboard['active_pregnancy']->id : '' }}">
    ...
    @if($dashboard && $dashboard['active_pregnancy'])
        @php $ob = $dashboard['obstetric_dashboard']; @endphp

        <div class="gyneco-alerts" id="gynecoAlerts">
            @foreach($dashboard['alerts'] as $alert)
                <div class="gyneco-alert gyneco-alert-{{ $alert['type'] }}">
        ...
        <div class="obstetric-grid">
            <div class="obstetric-card obstetric-card-primary">
                ...
                <div class="obstetric-card-body">
                    ...
                    <div class="obstetric-value">{{ $ob['gestational_age_display'] }}</div>
                    <div class="obstetric-sub">{{ $ob['trimester_label'] }}</div>
                </div>
                <div class="obstetric-progress">
                    <div class="obstetric-progress-bar" style="width: {{ min(100, round(($ob['gestational_age']['weeks'] ?? 0) / 42 * 100)) }}%"></div>
                </div>
            </div>
            ...
            <div class="obstetric-card obstetric-card-calendar">
                ...
                <div class="obstetric-card-body">
                    ...
                    <div class="obstetric-value">
                        {{ $ob['estimated_delivery_date'] ? $ob['estimated_delivery_date']->format('d/m/Y') : '-' }}
                    </div>
                    <div class="obstetric-sub">
                        @if($ob['days_until_delivery'] !== null)
                            @if($ob['days_until_delivery'] > 0)
                                Dans {{ $ob['days_until_delivery'] }} jours
                            @elseif($ob['days_until_delivery'] === 0)
                                Aujourd'hui
                            @else
                                Terme dépassé de {{ abs($ob['days_until_delivery']) }} jours
                            @endif
                        @else
                            Non calculée
                        @endif
            ...
            <div class="obstetric-card obstetric-card-risk obstetric-card-risk-{{ $ob['risk_level'] }}">
                ...
                <div class="obstetric-card-body">
                    ...
                    <div class="obstetric-value">
                        @switch($ob['risk_level'])
                            @case('low') Faible @break
                            @case('moderate') Modéré @break
                            @case('high') Élevé @break
                        @endswitch
                    </div>
                    <div class="obstetric-sub">{{ $ob['visit_count'] }} visite(s) prénatale(s)</div>
                </div>
            </div>
            ...
            <div class="obstetric-card obstetric-card-vitals">
                ...
                <div class="obstetric-card-body">
                    ...
                    <div class="obstetric-value">
                        {{ $ob['weight_gain']['total'] !== null ? '+' . $ob['weight_gain']['total'] . ' kg' : '-' }}
                    </div>
                    <div class="obstetric-sub">
                        Poids actuel: {{ $ob['weight_gain']['current'] ?? '-' }} kg
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="obstetric-sections">
            <div class="obstetric-section">
                ...
                <div class="milestone-list">
                    @forelse($ob['next_milestones'] as $milestone)
                        <div class="milestone-item">
                            <span class="milestone-week">{{ $milestone['week'] }} SA</span>
            ...
            <div class="obstetric-section">
                <h4>
                    ...
                    Examens requis ({{ $ob['trimester_label'] }})
                </h4>
                <div class="exam-checklist">
                    @foreach($ob['completed_exams'] as $key => $exam)
                        <div class="exam-check-item {{ $exam['completed'] ? 'is-done' : 'is-pending' }}">
                            ...
                            <span class="exam-check-label">{{ $exam['label'] }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="obstetric-section">
                ...
                <div class="serology-grid">
                    @foreach($ob['serology_summary'] as $name => $result)
                        <div class="serology-item {{ $result !== 'Non fait' ? 'is-done' : 'is-pending' }}">
                            <span class="serology-name">{{ ucfirst(str_replace('_', ' ', $name)) }}</span>
                            <span class="serology-result">{{ $result }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="obstetric-section">
                ...
                <div class="bp-trend-list">
                    @forelse($ob['blood_pressure_trend'] as $bp)
                        <div class="bp-trend-item {{ $bp['alert'] ? 'bp-alert' : '' }}">
            ...
            <div class="obstetric-section">
                ...
                <div class="ultrasound-list">
                    @forelse($ob['ultrasounds']->take(3) as $us)
                        <div class="ultrasound-item">
                            <div class="ultrasound-date">{{ $us->exam_date->format('d/m/Y') }}</div>
                            <div class="ultrasound-biometry">
                                @if($us->bip_mm)<span class="biom-tag">BIP: {{ $us->bip_mm }}mm</span>@endif
                                @if($us->hc_mm)<span class="biom-tag">PC: {{ $us->hc_mm }}mm</span>@endif
                                @if($us->ac_mm)<span class="biom-tag">PA: {{ $us->ac_mm }}mm</span>@endif
                                @if($us->fl_mm)<span class="biom-tag">LF: {{ $us->fl_mm }}mm</span>@endif
                                @if($us->efw_grams)<span class="biom-tag biom-tag-epf">EPF: {{ $us->efw_grams }}g</span>@endif
                            </div>
                            @if($us->conclusion)
                                <div class="ultrasound-conclusion">{{ Str::limit($us->conclusion, 80) }}</div>
                            @endif
                        </div>
                    @empty
            ...
            <div class="modal-body">
                <form id="quickEditRiskForm">
                    <div class="qe-risk-options">
                        @foreach(['low' => ['Faible', '#16a34a', '#f0fdf4'], 'moderate' => ['Modéré', '#d97706', '#fffbeb'], 'high' => ['Élevé', '#dc2626', '#fef2f2']] as $level => [$label, $color, $bg])
                            <label class="qe-risk-option {{ $ob['risk_level'] === $level ? 'active' : '' }}" style="border-color:{{ $ob['risk_level'] === $level ? $color : '#e2e8f0' }};background:{{ $ob['risk_level'] === $level ? $bg : '#fff' }}">
                                <input type="radio" name="risk_level" value="{{ $level }}" {{ $ob['risk_level'] === $level ? 'checked' : '' }} hidden>
                                <span class="qe-risk-dot" style="background:{{ $color }}"></span>
                                {{ $label }}
                            </label>
                        @endforeach
                    </div>
