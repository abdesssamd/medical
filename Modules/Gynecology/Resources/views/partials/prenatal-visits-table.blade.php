...
<section id="prenatal-visits-table" class="card gyneco-card" data-care-tab-panel="clinical">
    <div class="section-head">
        ...
        <span class="pnv-count-badge">{{ $visits->count() }} visite(s)</span>
    </div>
            ...
            <table class="pnv-table">
                ...
                <tbody>
                    @foreach($visits as $visit)
                        <tr class="{{ ($visit->blood_pressure_systolic >= 140 || $visit->blood_pressure_diastolic >= 90) ? 'pnv-row-alert' : '' }}">
                            <td class="pnv-cell-date">{{ $visit->visit_date->format('d/m/Y') }}</td>
                            <td class="pnv-cell-sa">
                                @if($visit->gestational_weeks_at_visit !== null)
                                    <span class="pnv-sa-badge">{{ $visit->gestational_weeks_at_visit }}+{{ $visit->gestational_days_at_visit ?? 0 }}</span>
                                @else
                            ...
                            <td class="pnv-cell-weight">
                                @if($visit->weight_kg)
                                    {{ $visit->weight_kg }} <small>kg</small>
                                @else
                                    <span class="pnv-muted">-</span>
                                @endif
                            </td>
                            <td class="pnv-cell-bp">
                                @if($visit->blood_pressure_systolic && $visit->blood_pressure_diastolic)
                                    <span class="{{ ($visit->blood_pressure_systolic >= 140 || $visit->blood_pressure_diastolic >= 90) ? 'pnv-bp-alert' : '' }}">
                            ...
                            <td class="pnv-cell-hu">
                                @if($visit->fundal_height_cm)
                                    {{ $visit->fundal_height_cm }} <small>cm</small>
                                @else
                                    <span class="pnv-muted">-</span>
                                @endif
                            </td>
                            <td class="pnv-cell-bcf">
                                @if($visit->fetal_heart_rate)
                                    <span class="pnv-bcf-value">{{ $visit->fetal_heart_rate }}</span> <small>bpm</small>
                                @else
                                    <span class="pnv-muted">-</span>
                                @endif
                            </td>
                            <td class="pnv-cell-actions">
                                <button type="button" class="pnv-action-btn pnv-btn-detail" title="Détails" data-visit-id="{{ $visit->id }}">
