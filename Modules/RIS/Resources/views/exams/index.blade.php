            ...
            <div class="ris26-panel">
                ...
                <form method="GET" class="ris26-filters">
                    ...
                    <div class="ris26-field" style="display: flex; align-items: flex-end; padding-bottom: 4px;">
                        ...
                        @if($filters['include_orphan'] && $orphanStudiesError)
                            <div style="margin-top:6px; font-size:0.78rem; color:#b91c1c; font-weight:700;">
                                <i class="ti ti-alert-triangle"></i> {{ $orphanStudiesError }}
                            </div>
                        @endif
                    </div>
            ...
            <section class="ris26-card">
                <div class="ris26-panel">
                    <div class="ris26-table-header">
                        ...
                        <span class="ris26-badge-count">{{ $orders->total() }} examen(s)</span>
                    </div>
                                ...
                                @forelse($orders as $order)
                                    ...
                                    <tr class="ris26-row" data-order-row data-order-id="{{ $order->id }}" data-order-status="{{ $order->status }}">
                                        <td>
                                            ...
                                            <div class="ris26-entity-meta">{{ optional($order->requested_at)->format('d/m/Y H:i') }}</div>
                                        </td>
                                        <td>
                                            <div class="ris26-patient-cell">
                                                <span class="ris26-patient-avatar">{{ $patientInitials }}</span>
                                                <div>
                                                    <div class="ris26-entity-title">{{ $order->patient?->full_name ?? 'Patient inconnu' }}</div>
                                                    <div class="ris26-entity-meta">{{ $order->patient?->medical_record_number }} {{ $order->patient?->phone ? '| '.$order->patient->phone : '' }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td colspan="6" style="padding: 24px 16px 8px;">
                                            <div style="display: flex; align-items: center; gap: 10px;">
                                                ...
                                                <span style="font-size: 0.76rem; text-transform: uppercase; letter-spacing: 0.08em; color: #92400e; font-weight: 800;">
                                                    Études PACS orphelines — {{ $orphanStudies->count() }} trouvée(s)
                                                </span>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    @foreach($orphanStudies as $study)
                                        ...
                                        <tr class="ris26-row ris26-row-orphan">
                                            <td>
                                                ...
                                                <div class="ris26-entity-meta">{{ $studyDate }}</div>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="ris26-row-actions">
                                                    ...
                                                    <form method="POST" action="{{ route('ris.exams.import-orphan') }}" style="display:inline;">
                                                        @csrf
                                                        <input type="hidden" name="orthanc_study_id" value="{{ $study->orthanc_study_id }}">
                                                        <input type="hidden" name="study_uid" value="{{ $study->study_uid }}">
                                                        <input type="hidden" name="patient_id" value="{{ $study->patient_id }}">
                                                        <input type="hidden" name="patient_name" value="{{ $study->patient_name }}">
                                                        <input type="hidden" name="accession_number" value="{{ $study->accession_number }}">
