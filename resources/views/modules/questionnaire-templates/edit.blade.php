...
@section('page_title', 'Éditer le Questionnaire')
...
<div class="edit-page">
    <div class="form-container">
        <form method="POST" action="{{ route('clinical.questionnaire-templates.update', $questionnaire->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="form-section">
                        ...
                        <select id="specialty_id" name="specialty_id">
                            ...
                            @foreach ($specialties as $specialty)
                                <option value="{{ $specialty->id }}" 
                                    {{ old('specialty_id', $questionnaire->specialty_id) == $specialty->id ? 'selected' : '' }}>
                                    {{ $specialty->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" 
                              placeholder="Description facultative du questionnaire...">{{ old('description', $questionnaire->description) }}</textarea>
                ...
                <div class="form-row">
                        ...
                        <select id="practitioner_id" name="practitioner_id">
                            ...
                            @foreach ($practitioners as $practitioner)
                                <option value="{{ $practitioner->id }}"
                                    {{ old('practitioner_id', $questionnaire->practitioner_id) == $practitioner->id ? 'selected' : '' }}>
                                    {{ $practitioner->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="group_name" name="group_name" 
                               placeholder="Ex: Bilan de Santé" value="{{ old('group_name', $questionnaire->group_name) }}">
            ...
            <div class="form-section">
                ...
                <div class="form-builder">
                    <div class="builder-header">
                        ...
                        <span style="font-size: 13px; color: #94a3b8;" id="fieldCount">{{ count($formattedFields) }} champ{{ count($formattedFields) > 1 ? 's' : '' }}</span>
                    </div>

                    <div id="fieldsList" class="fields-list">
                        @forelse ($formattedFields as $index => $field)
                            <div class="field-item" data-field-id="field_{{ $index }}" data-type="{{ $field['type'] }}">
                                ...
                                <div class="field-content">
                                    <div>
                                        ...
                                        <input type="text" class="field-label" placeholder="Ex: Avez-vous des allergies ?" value="{{ $field['label'] }}">
                                    </div>
                                    <div>
                                        <label style="margin-bottom: 4px; font-size: 12px;">Placeholder</label>
                                        <input type="text" class="field-placeholder" placeholder="Texte d'aide" value="{{ $field['placeholder'] ?? '' }}">
                                    </div>
                                    @if ($field['type'] !== 'yesno')
                                        <div>
                                            <label style="margin-bottom: 4px; font-size: 12px;">Texte d'aide</label>
                                            <input type="text" class="field-helpText" placeholder="Ex: Description supplémentaire" value="{{ $field['helpText'] ?? '' }}">
                                        </div>
                                    @endif
                                    @if (in_array($field['type'], ['select', 'radio', 'checkbox']))
                                        <div class="field-options">
                                            <label style="font-size: 12px;">Options</label>
                                            <div class="field-options-list">
                                                @forelse ($field['options'] as $option)
                                                    <div class="option-input">
                                                        <input type="text" class="field-option" placeholder="Option" value="{{ $option }}">
                                                        <button type="button" class="btn-remove-option" onclick="this.parentElement.remove(); updateFieldsInput();">Retirer</button>
                                                    </div>
                                                @empty
                                                    <div class="option-input">
                                                        <input type="text" class="field-option" placeholder="Option">
                                                        <button type="button" class="btn-remove-option" onclick="this.parentElement.remove(); updateFieldsInput();">Retirer</button>
                                                    </div>
                                                @endforelse
                                            </div>
                                    ...
                                    <div style="grid-column: 1 / -1; display: flex; gap: 12px; align-items: center;">
                                        <span class="field-type-badge">{{ $fieldTypes[$field['type']]['label'] ?? $field['type'] }}</span>
                                        <div class="checkbox-wrapper" style="margin-bottom: 0;">
                                            <input type="checkbox" class="field-required" id="required_field_{{ $index }}" {{ $field['required'] ? 'checked' : '' }}>
                                            <label for="required_field_{{ $index }}" style="margin: 0; font-weight: 500; font-size: 12px;">Obligatoire</label>
                                        </div>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    <div class="checkbox-wrapper">
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $questionnaire->is_active) ? 'checked' : '' }}>
                        <label for="is_active">Activer ce modèle</label>
                    </div>
            ...
            <div class="button-group">
                <a href="{{ route('clinical.questionnaire-templates.show', $questionnaire->id) }}" class="btn-secondary-modern">
                    <i class="ti ti-x"></i>
                    Annuler
                </a>
...
<script>
    let fieldCounter = {{ count($formattedFields) }};
    ...
    // Initialize fields input on page load
    updateFieldsInput();
