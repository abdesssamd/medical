...
<section id="patient-directory" class="card patient-directory-card patient-nav-footer">
    ...
    <script type="application/json" id="patientDirectoryJson">@json($patientDirectory ?? [])</script>
    <div class="d-none" id="patientDirectoryMeta"
        data-selected-patient-id="{{ (int) ($selectedPatientId ?? 0) }}"
        data-patient-view-base="{{ route('care.module3.index') }}"
        data-clinical-file-base="{{ url('/care/module-3/patients') }}"
        data-module2-base="{{ route('care.module2.index') }}"
        data-new-patient="{{ request()->boolean('new_patient') ? '1' : '0' }}"></div>
</section>
...
<div class="clinical-shell">
    <aside class="clinical-sidebar card clinical-context-card">
        <div class="sidebar-head">
            ...
            @if($selectedPatient)
                ...
                <p>MRN {{ $selectedPatient->medical_record_number }} • {{ $selectedPatient->age }} ans</p>
            @else
        ...
        @if($selectedPatient)
            <div class="sidebar-context-metrics">
                <div class="context-metric">
                    <span>Alertes</span>
                    <strong>{{ $patientAlerts->count() }}</strong>
                </div>
                <div class="context-metric">
                    <span>Solde estime</span>
                    <strong>{{ number_format($estimatedBalance, 2, ',', ' ') }} MAD</strong>
                </div>
            </div>
            <div class="sidebar-alert-list">
                @forelse($patientAlerts as $alert)
                    <span class="alert-pill alert-pill-danger">{{ $alert }}</span>
                @empty
                    <span class="alert-pill alert-pill-success">Aucune alerte critique</span>
                @endforelse
            </div>
            <div class="sidebar-actions">
                <a class="btn btn-primary w-100" href="{{ route('care.module3.export', ['patientId' => $selectedPatientId]) }}" target="_blank">Exporter PDF dossier</a>
                ...
                <a class="btn btn-outline-secondary w-100 mt-2" href="{{ route('care.module2.index') }}">Flux patient live</a>
            </div>
        @endif

        <nav class="clinical-tab-rail" aria-label="Navigation clinique">
            <button type="button" class="clinical-tab-link {{ $currentTab === 'overview' ? 'active' : '' }}" data-care-tab-target="overview">Vue generale</button>
            <button type="button" class="clinical-tab-link {{ $currentTab === 'clinical' ? 'active' : '' }}" data-care-tab-target="clinical">Examen clinique</button>
            <button type="button" class="clinical-tab-link {{ $currentTab === 'care' ? 'active' : '' }}" data-care-tab-target="care">Saisie de soins</button>
            <button type="button" class="clinical-tab-link {{ $currentTab === 'documents' ? 'active' : '' }}" data-care-tab-target="documents">Documents</button>
            @if(in_array($currentSpecialtyCode, ['REHAB', 'OMNI']))
            <button type="button" class="clinical-tab-link {{ $currentTab === 'rehab' ? 'active' : '' }}" data-care-tab-target="rehab">Rééducation</button>
            @endif
            @if(in_array($currentSpecialtyCode, ['INTMED', 'OMNI']))
            <button type="button" class="clinical-tab-link {{ $currentTab === 'internal-medicine' ? 'active' : '' }}" data-care-tab-target="internal-medicine">Médecine Interne</button>
            @endif
        </nav>
    </aside>
    ...
    <div class="clinical-main">
        <section class="card clinical-sticky-header">
            <div class="sticky-header-top">
                <div>
                    ...
                    @if($selectedPatient)
                        <h2 class="sticky-patient-name">{{ $selectedPatient->full_name }}</h2>
                        <div class="sticky-patient-meta">{{ $selectedPatient->age }} ans • {{ $selectedPatient->medical_record_number }} • {{ $selectedPatient->phone ?: 'Telephone non renseigne' }}</div>
                    @else
            ...
            <div class="clinical-quick-actions" aria-label="Actions rapides du dossier patient">
                <div class="clinical-quick-actions-desktop d-none d-md-flex" role="toolbar" aria-label="Actions rapides">
                    ...
                    @endphp
                    @if($risEnabled)
                        <a href="{{ route('ris.exams.index') }}" class="btn btn-outline-dark btn-sm">
        ...
        <!--div class="clinical-tabbar card">
            <button type="button" class="clinical-tab-button {{ $currentTab === 'overview' ? 'active' : '' }}" data-care-tab-target="overview">Vue gÃ©nÃ©rale</button>
            <button type="button" class="clinical-tab-button {{ $currentTab === 'clinical' ? 'active' : '' }}" data-care-tab-target="clinical">Examen clinique</button>
            <button type="button" class="clinical-tab-button {{ $currentTab === 'care' ? 'active' : '' }}" data-care-tab-target="care">Saisie de soins</button>
            <button type="button" class="clinical-tab-button {{ $currentTab === 'documents' ? 'active' : '' }}" data-care-tab-target="documents">Documents</button>
        </div-->
        ...
        <section id="overview" class="card overview-card" data-care-tab-panel="overview">
            ...
            @if($selectedPatient)
                <div id="module3-patient-context"
                     ...
                     data-patient-age="{{ $selectedPatient->age }}"
                ...
                <div class="patient-header-tech">
                    <div class="patient-id">
                        @if($selectedPatient->patient_photo_path)
                            <img src="{{ asset($selectedPatient->patient_photo_path) }}" class="avatar" alt="Photo patient">
                        @else
                            <div class="avatar">{{ strtoupper(substr($selectedPatient->first_name, 0, 1) . substr($selectedPatient->last_name, 0, 1)) }}</div>
                        @endif
                        <div>
                            <h2>{{ $selectedPatient->full_name }}</h2>
                            <div class="muted">MRN: {{ $selectedPatient->medical_record_number }} | Age: {{ $selectedPatient->age }} ans | Sexe: {{ strtoupper((string) ($selectedPatient->gender ?: '-')) }} | Groupe: {{ $selectedPatient->blood_group ?: '-' }} | Tel: {{ $selectedPatient->phone ?: '-' }}</div>
                            <div class="muted">Taille: {{ $selectedPatient->height_cm ? number_format((float) $selectedPatient->height_cm, 1, ',', ' ') . ' cm' : '-' }} | Poids: {{ $selectedPatient->weight_kg ? number_format((float) $selectedPatient->weight_kg, 1, ',', ' ') . ' kg' : '-' }} | IMC: {{ $bmi !== null ? number_format((float) $bmi, 1, ',', ' ') : '-' }}</div>
                        </div>
                    </div>
                    <div class="patient-widget">
                        <div class="widget-title">Prochain rendez-vous</div>
                        @if($nextAppointment)
                            <div class="widget-value">{{ \Carbon\Carbon::parse($nextAppointment->appointment_date)->format('d/m/Y') }} {{ \Illuminate\Support\Str::of($nextAppointment->start_time)->substr(0,5) }}</div>
                            <div class="widget-sub">Praticien ID: {{ $nextAppointment->professional_id }}</div>
                        @else
                ...
                <div class="overview-priority-layout">
                            ...
                            <div class="patient-history-groups">
                                <div class="patient-history-group">
                                    ...
                                    <div class="patient-history-badges" id="historyAllergiesBadges">
                                        @forelse($patientAllergies as $item)
                                            <span class="history-badge history-badge-danger">{{ $item }}</span>
                                        @empty
                                ...
                                <div class="patient-history-group">
                                    ...
                                    <div class="patient-history-badges" id="historyMedicalBadges">
                                        @forelse($patientMedicalHistory as $item)
                                            <span class="history-badge history-badge-warning">{{ $item }}</span>
                                        @empty
                                ...
                                <div class="patient-history-group">
                                    ...
                                    <div class="patient-history-badges" id="historyRiskBadges">
                                        @forelse($patientCriticalConditions as $item)
                                            <span class="history-badge history-badge-warning">{{ $item }}</span>
                                        @empty
                    ...
                    <div class="overview-priority-main">
                        <div class="allergy-bar {{ $criticalBannerItems->isNotEmpty() ? 'has-alert' : '' }}">
                            <strong>Sante critique:</strong>
                            @if($criticalBannerItems->isNotEmpty())
                                <span>{{ $criticalBannerItems->implode(' | ') }}</span>
                            @else
                                <span>Aucune allergie ou pathologie a risque signalee</span>
                            @endif
                        </div>

                        @if(!empty($selectedPatientNeedsPreventiveRecall))
                            <div class="preventive-recall-alert">
                                <strong>Patient Ã  rappeler pour dÃ©tartrage annuel.</strong>
                                <span>
                                    DerniÃ¨re visite: {{ $selectedPatientLastVisitAt ?: 'Aucune visite enregistrÃ©e' }}
                                </span>
                            </div>
                        @endif
                    </div>
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        <div class="p-3 border rounded-3 h-100">
                            ...
                            <div class="small">MRN: <strong>{{ $selectedPatient->medical_record_number }}</strong></div>
                            <div class="small">Nom: {{ $selectedPatient->full_name }}</div>
                            <div class="small">Date naissance: {{ optional($selectedPatient->date_of_birth)->format('d/m/Y') }}</div>
                            <div class="small">Sexe: {{ strtoupper((string) ($selectedPatient->gender ?: '-')) }}</div>
                            <div class="small">Groupe sanguin: {{ $selectedPatient->blood_group ?: '-' }}</div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="p-3 border rounded-3 h-100 inline-editable-card" data-history-type="personal">
                            ...
                            @if(collect($selectedPatient->personal_history ?? [])->isNotEmpty())
                                <div class="history-items-list" id="personal-history-items">
                                    @foreach(($selectedPatient->personal_history ?? []) as $item)
                                        <div class="history-item history-item-tag" data-item="{{ $item }}" data-history-type="personal">
                                            <span>{{ $item }}</span>
                                            <button type="button" class="btn-close-inline" data-history-type="personal" data-item="{{ $item }}" title="Supprimer">âœ•</button>
                                        </div>
                                    @endforeach
                                </div>
                            @else
                    ...
                    <div class="col-md-4">
                        <div class="p-3 border rounded-3 h-100 inline-editable-card" data-history-type="family">
                            ...
                            @if(collect($selectedPatient->family_history ?? [])->isNotEmpty())
                                <div class="history-items-list" id="family-history-items">
                                    @foreach(($selectedPatient->family_history ?? []) as $item)
                                        <div class="history-item history-item-tag" data-item="{{ $item }}" data-history-type="family">
                                            <span>{{ $item }}</span>
                                            <button type="button" class="btn-close-inline" data-history-type="family" data-item="{{ $item }}" title="Supprimer">âœ•</button>
                                        </div>
                                    @endforeach
                                </div>
                            @else
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-sm mb-0">
                                                ...
                                                @forelse($consultations as $consultation)
                                                    <tr>
                                                        <td>{{ optional($consultation->consultation_date)->format('d/m/Y') }}</td>
                                    ...
                                    <div class="accordion-body p-0">
                                        <div class="table-responsive">
                                            <table class="table table-sm mb-0">
                                                ...
                                                <tbody>
                                                @forelse($questionnaireResponses as $response)
                                                    <tr>
                                                        <td>{{ optional($response->answered_at)->format('d/m/Y') }}</td>
                                                        <td>{{ $response->questionnaire?->name ?? 'Questionnaire' }}</td>
                                                        <td><span class="badge bg-azure-lt">{{ $response->questionnaire?->specialty?->name ?? '-' }}</span></td>
                                                        <td><button type="button" class="btn btn-sm btn-outline-primary">Voir</button></td>
                                                    </tr>
                                                @empty
        ...
        <div id="patientSlideover" class="patient-slideover" aria-hidden="true">
            ...
            <aside class="patient-slideover-panel" role="dialog" aria-modal="true" aria-labelledby="newPatientTitle">
                ...
                <form method="POST" action="{{ route('care.module3.patients.store') }}" id="newPatientForm" class="slideover-form" enctype="multipart/form-data" data-store-action="{{ route('care.module3.patients.store') }}" data-update-base="{{ url('/care/module-3/patients') }}" data-no-spa="true" novalidate>
                    @csrf
        ...
        @if($selectedPatientId && $selectedPatient)
            ...
            @if(in_array($currentSpecialtyCode, ['GYNECO', 'OMNI']))
                @include('gynecology::partials.obstetric-dashboard', ['dashboard' => $gynecologyDashboard, 'selectedPatientId' => $selectedPatientId])
                @include('gynecology::partials.prenatal-visits-table', ['dashboard' => $gynecologyDashboard, 'selectedPatientId' => $selectedPatientId])
                @include('gynecology::partials.fetal-biometry-chart', ['dashboard' => $gynecologyDashboard, 'selectedPatientId' => $selectedPatientId])
                @include('gynecology::partials.gynecological-history', ['history' => $gynecologyDashboard['gynecological_history'] ?? null, 'selectedPatientId' => $selectedPatientId])
                @include('gynecology::partials.pregnancy-form', ['pregnancy' => $gynecologyDashboard['active_pregnancy'] ?? null, 'selectedPatientId' => $selectedPatientId])
                @include('gynecology::partials.fetal-biometry-form', ['pregnancy' => $gynecologyDashboard['active_pregnancy'] ?? null, 'selectedPatientId' => $selectedPatientId])
            @endif

            @if(in_array($currentSpecialtyCode, ['PEDIA', 'OMNI']))
                @include('pediatrics::partials.neonatal-form', ['birthHistory' => $pediatricsDashboard['birth_history'] ?? null, 'selectedPatientId' => $selectedPatientId])
                @include('pediatrics::partials.growth-charts', ['growthRecords' => $pediatricsDashboard['growth_records'] ?? collect(), 'growthChartData' => $pediatricsDashboard['growth_chart_data'] ?? [], 'selectedPatientId' => $selectedPatientId])
                @include('pediatrics::partials.vaccination-grid', ['vaccinationSchedule' => $pediatricsVaccinationSchedule ?? [], 'vaccinationSummary' => $pediatricsVaccinationSummary ?? [], 'selectedPatientId' => $selectedPatientId])
            @endif

            @if(in_array($currentSpecialtyCode, ['BURNS', 'OMNI']))
                @include('burns::partials.body-mapping', ['admission' => $burnsDashboard['admission'] ?? null, 'latestAssessment' => $burnsDashboard['latest_assessment'] ?? null, 'selectedPatientId' => $selectedPatientId])
                @include('burns::partials.resuscitation-dashboard', ['admission' => $burnsDashboard['admission'] ?? null, 'fluidResuscitation' => $burnsDashboard['fluid_resuscitation'] ?? null, 'fluidStatus' => $burnsDashboard['fluid_status'] ?? [], 'selectedPatientId' => $selectedPatientId])
                @include('burns::partials.wound-gallery', ['admission' => $burnsDashboard['admission'] ?? null, 'woundEvolutions' => $burnsDashboard['wound_evolutions'] ?? collect(), 'selectedPatientId' => $selectedPatientId])
            @endif

            @if(in_array($currentSpecialtyCode, ['REHAB', 'OMNI']))
                @include('rehab::partials.doctor-dashboard', ['prescriptions' => $rehabDashboard['prescriptions'] ?? collect(), 'activePrescription' => $rehabDashboard['active_prescription'] ?? null, 'stats' => $rehabDashboard['stats'] ?? [], 'evaluations' => $rehabDashboard['evaluations'] ?? collect(), 'sessions' => $rehabDashboard['sessions'] ?? collect(), 'selectedPatientId' => $selectedPatientId])
                @include('rehab::partials.physio-session-tracker', ['activePrescription' => $rehabDashboard['active_prescription'] ?? null, 'stats' => $rehabDashboard['stats'] ?? [], 'sessions' => $rehabDashboard['sessions'] ?? collect(), 'selectedPatientId' => $selectedPatientId])
            @endif

            @if(in_array($currentSpecialtyCode, ['INTMED', 'OMNI']))
                @include('internal-medicine::partials.chronic-diseases', ['chronicConditions' => $internalMedicineDashboard['chronic_conditions'] ?? collect(), 'selectedPatientId' => $selectedPatientId])
                @include('internal-medicine::partials.biology-dashboard', ['labChartData' => $internalMedicineDashboard['lab_chart_data'] ?? [], 'selectedPatientId' => $selectedPatientId])
                @include('internal-medicine::partials.clinical-scores', ['scoresSummary' => $internalMedicineDashboard['scores_summary'] ?? [], 'clinicalScores' => $internalMedicineDashboard['clinical_scores'] ?? null, 'selectedPatientId' => $selectedPatientId])
            @endif

            @if(in_array($currentSpecialtyCode, ['DENTAL', 'OMNI']))
            ...
            <section id="consultation-entry" class="card" data-care-tab-panel="care">
                ...
                <div class="soap-context-strip">
                    ...
                    <div class="soap-context-note card">
                        ...
                        <div class="soap-trace-slot">
                            <input class="form-control form-control-sm @error('sterilization_pouch_code') is-invalid @enderror" name="sterilization_pouch_code" value="{{ old('sterilization_pouch_code') }}" placeholder="Code sachet (ex: BATCH-2026-001-003)">
                            @error('sterilization_pouch_code')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                            <input class="form-control form-control-sm @error('sterilization_notes') is-invalid @enderror" name="sterilization_notes" value="{{ old('sterilization_notes') }}" placeholder="Notes traçabilité (cycle, opérateur)">
                            @error('sterilization_notes')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                        </div>
                    </div>
                </div>
                ...
                @if($selectedPatient)
                    <div class="consultation-flash">
                        <div><strong>Allergies</strong>: {{ $allergies->implode(', ') ?: 'Aucune' }}</div>
                        <div><strong>Antécédents</strong>: {{ collect($selectedPatient->medical_history ?? [])->implode(', ') ?: 'Aucun' }}</div>
                        <div><strong>Médicaments</strong>: {{ collect($selectedPatient->current_medications ?? [])->implode(', ') ?: 'Aucun' }}</div>
                    </div>
                @endif

                @if($errors->any() && old('_form_context') === 'consultation')
                ...
                <div class="modal-dialog modal-xl modal-dialog-scrollable">
                    <div class="modal-content border-0 shadow-lg">
                        ...
                        <div class="modal-body p-4">
                            <form method="POST" action="{{ route('care.module3.consultations.store', ['patientId' => $selectedPatientId]) }}" class="row g-3" enctype="multipart/form-data" id="consultationForm">
                                @csrf
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input class="form-control @error('consultation_date') is-invalid @enderror" type="date" name="consultation_date" value="{{ old('consultation_date', now()->toDateString()) }}" required>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-select @error('consultation_type') is-invalid @enderror" name="consultation_type" required>
                                        @foreach($consultationTypes as $type)
                                            <option value="{{ $type['value'] }}" @selected(old('consultation_type') === $type['value'])>{{ $type['label'] }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-select @error('consultation_status') is-invalid @enderror" name="consultation_status" required>
                                        <option value="attendu" @selected(old('consultation_status', 'attendu') === 'attendu')>Attendu</option>
                                        <option value="en_soin" @selected(old('consultation_status') === 'en_soin')>En soin</option>
                                        <option value="termine" @selected(old('consultation_status') === 'termine')>Terminé</option>
                                    </select>
                                    @error('consultation_status')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label fw-semibold">Praticien <span class="text-danger">*</span></label>
                                    <select class="form-select @error('practitioner_id') is-invalid @enderror" name="practitioner_id" required>
                                        <option value="">Sélectionner</option>
                                        @foreach($practitioners as $practitioner)
                                            <option value="{{ $practitioner->id }}" @selected((string) old('practitioner_id', auth()->id()) === (string) $practitioner->id)>{{ $practitioner->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input class="form-control @error('consultation_reason') is-invalid @enderror" name="consultation_reason" list="consultationMotifsList" value="{{ old('consultation_reason') }}" placeholder="Douleur, détartrage, urgence..." required>
                                    <datalist id="consultationMotifsList">
                                        @foreach(collect($consultationMotifs ?? [])->merge(['Douleur', 'Détartrage', 'Urgence', 'Contrôle', 'Bilan initial'])->unique() as $motif)
                                            <option value="{{ $motif }}"></option>
                                        @endforeach
                                    </datalist>
                                    @error('consultation_reason')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold">Rattacher à un RDV</label>
                                    <select class="form-select @error('appointment_id') is-invalid @enderror" name="appointment_id">
                                        <option value="">Aucun</option>
                                        @foreach($appointmentsForPatient as $appointment)
                                            <option value="{{ $appointment->id }}" @selected((string) old('appointment_id') === (string) $appointment->id)>{{ optional($appointment->appointment_date)->format('d/m/Y') }} {{ substr($appointment->start_time, 0, 5) }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <textarea class="form-control @error('chief_complaint') is-invalid @enderror" name="chief_complaint" rows="2" placeholder="Motif exprimé par le patient...">{{ old('chief_complaint') }}</textarea>
                                    @error('chief_complaint')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold">Anamnèse</label>
                                    <textarea class="form-control @error('anamnesis') is-invalid @enderror" name="anamnesis" rows="2" placeholder="Historique, contexte, évolution...">{{ old('anamnesis') }}</textarea>
                                ...
                                <div class="col-md-8">
                                    ...
                                    <textarea class="form-control @error('clinical_exam') is-invalid @enderror" name="clinical_exam" rows="3" placeholder="Constats objectifs, signes cliniques, résultats observés...">{{ old('clinical_exam') }}</textarea>
                                    @error('clinical_exam')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label fw-semibold">Constantes vitales</label>
                                    <div class="row g-2">
                                        <div class="col-12">
                                            <input class="form-control @error('vital_bp') is-invalid @enderror" name="vital_bp" value="{{ old('vital_bp') }}" placeholder="Tension (ex: 120/80)">
                                            @error('vital_bp')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                        </div>
                                        <div class="col-12">
                                            <input class="form-control @error('vital_pulse') is-invalid @enderror" type="number" name="vital_pulse" value="{{ old('vital_pulse') }}" min="20" max="250" placeholder="Pouls (bpm)">
                                            @error('vital_pulse')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                        </div>
                                        <div class="col-12">
                                            <input class="form-control @error('vital_weight') is-invalid @enderror" type="number" step="0.1" name="vital_weight" value="{{ old('vital_weight') }}" min="1" max="500" placeholder="Poids (kg)">
                                            @error('vital_weight')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input class="form-control @error('diagnosis_label') is-invalid @enderror" name="diagnosis_label" id="diagnosisLabelInput" list="diagnosisCatalogList" value="{{ old('diagnosis_label') }}" placeholder="Commencez à taper un diagnostic...">
                                    <datalist id="diagnosisCatalogList">
                                        @foreach($diagnosisCatalog as $diagnosis)
                                            <option value="{{ $diagnosis['label'] }}" label="{{ $diagnosis['code'] }}"></option>
                                        @endforeach
                                    </datalist>
                                    <input type="hidden" name="diagnosis_code" id="diagnosisCodeInput" value="{{ old('diagnosis_code') }}">
                                    ...
                                    @error('diagnosis_label')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                                    @error('diagnosis_code')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <div id="consultationObservationsEditor" class="rich-editor @error('observations') is-invalid @enderror" contenteditable="true"></div>
                                    <textarea class="d-none" name="observations" id="consultationObservationsInput">{{ old('observations') }}</textarea>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <textarea class="form-control @error('recommendations') is-invalid @enderror" name="recommendations" rows="3" placeholder="Conseils, suivi, réévaluation...">{{ old('recommendations') }}</textarea>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <textarea class="form-control @error('prescription') is-invalid @enderror" name="prescription" rows="3" placeholder="Traitement prescrit...">{{ old('prescription') }}</textarea>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input class="form-control @error('attachments') is-invalid @enderror @error('attachments.*') is-invalid @enderror" type="file" name="attachments[]" accept=".jpg,.jpeg,.png,.webp,.pdf" multiple>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <textarea class="form-control @error('notes') is-invalid @enderror" name="notes" rows="3" placeholder="Éléments complémentaires, consignes, suivi...">{{ old('notes') }}</textarea>
                                    @error('notes')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>

                                <div class="col-12">
                                    <label class="form-label fw-semibold">Constantes additionnelles (optionnel)</label>
                                    <textarea class="form-control @error('vital_signs') is-invalid @enderror" name="vital_signs" rows="2" placeholder="Ex: Température: 37.2&#10;SpO2: 98%">{{ old('vital_signs') }}</textarea>
                                    @error('vital_signs')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>

                                <div class="col-12">
                                    <div class="form-check">
                                        <input class="form-check-input @error('consent_obtained') is-invalid @enderror" type="checkbox" name="consent_obtained" value="1" @checked(old('consent_obtained')) required>
                                        <label class="form-check-label fw-semibold">Consentement éclairé obtenu <span class="text-danger">*</span></label>
                                    </div>
                                    @error('consent_obtained')<div class="text-danger small">{{ $message }}</div>@enderror
                                </div>
                            </form>
                        </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-sm mb-0">
                                        ...
                                        @forelse($prescriptions as $rx)
                                            <tr>
                                                <td>{{ optional($rx->issued_at)->format('d/m/Y H:i') }}</td>
                                                <td>{{ $rx->prescription_number }}</td>
                                                <td>{{ $rx->practitioner?->name ?: '-' }}</td>
                                                <td><a class="btn btn-sm btn-outline-primary" target="_blank" href="{{ route('care.module3.prescriptions.pdf', ['prescription' => $rx->id]) }}">PDF</a></td>
                                            </tr>
                                        @empty
                                    ...
                                    <table class="table table-sm mb-0">
                                        ...
                                        @forelse($questionnaireResponses as $response)
                                            <tr>
                                                <td>{{ optional($response->answered_at)->format('d/m/Y') }}</td>
                                                <td>{{ $response->questionnaire?->name ?? 'Questionnaire' }}</td>
                                                <td>{{ $response->practitioner?->name ?: '-' }}</td>
                                                <td><button type="button" class="btn btn-sm btn-outline-primary">Voir</button></td>
                                            </tr>
                                        @empty
            ...
            <section id="timeline" class="card" data-care-tab-panel="clinical">
                ...
                <form method="GET" class="row g-2 align-items-end mb-3" id="timelineFilterForm">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="date" class="form-control" name="timeline_from" value="{{ $timelineFilters['timeline_from'] ?? '' }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Au</label>
                        <input type="date" class="form-control" name="timeline_to" value="{{ $timelineFilters['timeline_to'] ?? '' }}">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Type de soin</label>
                        <select class="form-select" name="timeline_type">
                            <option value="all" @selected(($timelineFilters['timeline_type'] ?? 'all') === 'all')>Tous</option>
                            <option value="consultation" @selected(($timelineFilters['timeline_type'] ?? '') === 'consultation')>Consultations</option>
                            <option value="procedure" @selected(($timelineFilters['timeline_type'] ?? '') === 'procedure')>Actes</option>
                            <option value="treatment_plan" @selected(($timelineFilters['timeline_type'] ?? '') === 'treatment_plan')>Plans</option>
                            <option value="imaging" @selected(($timelineFilters['timeline_type'] ?? '') === 'imaging')>Imagerie</option>
                        </select>
                    </div>
                    <div class="col-md-2">
                ...
                <div class="timeline-vertical">
                    @forelse(($timeline['events'] ?? []) as $event)
                        <article class="timeline-item">
                            ...
                            <div class="timeline-content" data-timeline-row data-searchable="{{ \Illuminate\Support\Str::lower(($event['searchable'] ?? collect([$event['label'] ?? '', $event['diagnosis'] ?? '', $event['practitioner'] ?? '', $event['consultation'] ?? '', $event['notes'] ?? ''])->filter()->implode(' | '))) }}">
                                <header>
                                    <strong>{{ $event['label'] }}</strong>
                                    <span class="badge bg-indigo-lt">{{ $event['type'] }}</span>
                                </header>
                                <div class="meta d-flex flex-wrap gap-2 align-items-center">{{ $event['date'] ?? '-' }} | Motif: {{ $event['consultation_reason'] ?? $event['label'] ?? '-' }} | Praticien: {{ $event['practitioner'] ?? '-' }} | Statut: {{ $event['status'] ?? '-' }}</div>
                                @if($event['type'] === 'consultation')
                                    <div class="meta">Type: {{ $event['consultation_type'] ?? '-' }} | Paiement: {{ $event['payment_status'] ?? '-' }}</div>
                                    <div class="meta">Diagnostic: {{ $event['diagnosis'] ?? '-' }}</div>
                                    @if(!empty($event['invoice_number']))<div class="meta">Facture: {{ $event['invoice_number'] }}</div>@endif
                                @endif
                                @if(!empty($event['tooth_number']))<div class="meta">Dent: {{ $event['tooth_number'] }}</div>@endif
                                @if(!empty($event['specialty']))<div class="meta">Specialite: {{ $event['specialty'] }}</div>@endif
                                @if($event['type'] === 'treatment_plan')
                                    <div class="meta">Budget estime: {{ number_format((float) ($event['estimated_cost'] ?? 0), 2, ',', ' ') }} MAD</div>
                                @endif
            ...
            <section id="procedures-history" class="card" data-care-tab-panel="care">
                ...
                <div class="consultation-timeline">
                    @forelse($consultations as $consultation)
                        ...
                        @endphp
                        <details class="consultation-group" {{ $loop->first ? 'open' : '' }}>
                            <summary class="consultation-group-header">
                                <div class="consultation-group-meta">
                                    <span class="consultation-date {{ $isToday ? 'text-primary fw-bold' : '' }}">
                                        {{ $consultation->consultation_date?->format('d/m/Y') ?? 'N/A' }}
                                        @if($isToday)
                                            <span class="badge bg-primary-lt ms-1">Aujourd'hui</span>
                                        @endif
                                    </span>
                                    <span class="consultation-type badge bg-secondary-lt">{{ $consultation->consultation_type ?? 'Consultation' }}</span>
                                    <span class="consultation-practitioner text-muted small">
                                        ...
                                        {{ $consultation->practitioner?->name ?? 'N/A' }}
                                    </span>
                                    <span class="badge bg-info-lt ms-auto">
                                        {{ $consProcedures->count() }} acte(s)
                                    </span>
                                </div>
                                @if($consultation->consultation_reason)
                                    <div class="consultation-reason small text-muted mt-1">
                                        <strong>Motif:</strong> {{ $consultation->consultation_reason }}
                                    </div>
                                @endif
                            </summary>

                            <div class="consultation-procedures">
                                @forelse($consProcedures as $procedure)
                                    <div class="procedure-card">
                                        <div class="procedure-card-head">
                                            <strong>{{ $procedure->name }}</strong>
                                            <span class="badge {{ match($procedure->status) {
                                        ...
                                        <div class="procedure-card-body">
                                            <div class="procedure-detail">
                                                ...
                                                <span>Dent <strong>{{ $procedure->tooth_number }}</strong></span>
                                            </div>
                                            @if($procedure->tooth_surfaces)
                                                <div class="procedure-detail">
                                                    ...
                                                    <span>Surfaces: {{ is_array($procedure->tooth_surfaces) ? implode(', ', $procedure->tooth_surfaces) : $procedure->tooth_surfaces }}</span>
                                                </div>
                                            @endif
                                            @if($procedure->specialty)
                                                <div class="procedure-detail">
                                                    ...
                                                    <span>{{ $procedure->specialty->name }}</span>
                                                </div>
                                            @endif
                                            @if($procedure->practitioner)
                                                <div class="procedure-detail">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                                                    <span>{{ $procedure->practitioner->name }}</span>
                                                </div>
                                            @endif
                                            @if($procedure->price > 0)
                                                <div class="procedure-detail">
                                                    ...
                                                    <span>{{ number_format((float) $procedure->price, 2, ',', ' ') }} MAD</span>
                                                </div>
                                            @endif
                                            @if($procedure->notes)
                                                <div class="procedure-notes mt-1 small text-muted">
                                                    <em>{{ $procedure->notes }}</em>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
            ...
            <section id="imaging" class="card" data-care-tab-panel="documents">
                ...
                <div class="collapse" id="radiologyRequestCollapse">
                    <div class="border rounded p-3 mb-3 bg-light">
                        ...
                        <form method="POST" action="{{ route('care.module3.radiology-requests.store', ['patientId' => $selectedPatientId]) }}" class="row g-2">
                            @csrf
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select" name="prescribing_physician_id" required>
                                    ...
                                    @foreach($practitioners as $practitioner)
                                        <option value="{{ $practitioner->id }}" @selected(auth()->id() === $practitioner->id)>{{ $practitioner->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-12">
                    ...
                    <table class="table table-sm align-middle">
                        ...
                        <tbody>
                            @forelse($radiologyRequests as $request)
                                ...
                                <tr>
                                    <td>
                                        <div><strong>{{ $request->accession_number }}</strong></div>
                                        <div class="small text-secondary">UID: {{ $request->study_instance_uid }}</div>
                                    </td>
                                    <td>
                                        <div>{{ $request->exam_type }}</div>
                                        <div class="small text-secondary">{{ $request->anatomical_region }}</div>
                                    </td>
                                    <td>
                                        <span class="badge {{ $request->priority === 'urgent' ? 'bg-red-lt' : 'bg-yellow-lt' }}">{{ strtoupper($request->priority) }}</span>
                                    </td>
                                    <td><span class="badge {{ $statusTone }}">{{ $statusLabel }}</span></td>
                                    <td>{{ $request->scheduled_station_ae_title }} / {{ $request->target_modality }}</td>
                                    <td>{{ $request->prescribingPhysician?->name ?: '-' }}</td>
                                    <td>
                                        @if($isReady && $viewerUrl)
                                            <a href="{{ $viewerUrl }}" target="_blank" rel="noopener" class="btn btn-sm btn-outline-primary">Voir</a>
                                        @else
                ...
                <form method="POST" action="{{ route('care.module3.imaging.store', ['patientId' => $selectedPatientId]) }}" enctype="multipart/form-data" class="upload-box" id="dropZoneForm">
                    @csrf
                ...
                <div class="imaging-cards mt-3">
                    @forelse(($manifest['items'] ?? []) as $item)
                        ...
                        <article class="imaging-card">
                            <div>
                                <div><strong>{{ strtoupper($item['modality']) }}</strong> <span class="badge bg-blue-lt">#{{ $item['id'] }}</span></div>
                                <div class="muted small">{{ $item['study_uid'] ?: 'Sans Study UID' }}</div>
                                <div class="muted small">{{ \Illuminate\Support\Str::limit($item['file_path'], 70) }}</div>
                            </div>
                            <div class="imaging-actions">
                                @if($isImagePreview)
                                    <button type="button" class="btn btn-sm btn-outline-primary btn-preview" data-src="{{ asset($item['file_path']) }}">Previsualiser</button>
                                @else
            ...
            @if($risEnabled && ($risOrders ?? collect())->isNotEmpty())
            <section id="ris-examens" class="card" data-care-tab-panel="documents">
                <div class="section-head d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <h3 class="mb-0">RIS Examens (Orthanc)</h3>
                    <a href="{{ route('ris.exams.index') }}" class="btn btn-sm btn-outline-dark">
                    ...
                    <table class="table table-sm align-middle">
                        ...
                        <tbody>
                            @foreach($risOrders as $order)
                                ...
                                @endphp
                                <tr>
                                    <td><code>{{ $order->accession_number }}</code></td>
                                    <td>
                                        <div class="fw-semibold">{{ $order->procedure?->label ?: $order->requested_procedure_description }}</div>
                                        <div class="small text-secondary">{{ Str::limit($order->clinical_indication, 60) }}</div>
                                    </td>
                                    <td>{{ $order->modality?->name ?: '-' }}</td>
                                    <td><span class="badge {{ $statusBadge }}">{{ $statusLabel }}</span></td>
                                    <td class="small">{{ $order->requested_at?->format('d/m/Y H:i') }}</td>
                                    <td>
                                        @if($isReady && $viewerUrl)
                                            <a href="{{ $viewerUrl }}" target="_blank" rel="noopener" class="btn btn-sm btn-outline-primary">Viewer</a>
                                        @elseif($isReady)
                                            <a href="{{ route('ris.exams.show', $order) }}" class="btn btn-sm btn-outline-secondary">Ouvrir</a>
                                        @else
                                            <span class="text-secondary small">-</span>
                                        @endif
                                    </td>
                    ...
                    @forelse($treatmentPlans as $plan)
                        ...
                        <article class="treatment-card">
                            <header>
                                ...
                                <span class="badge bg-indigo-lt">{{ $plan->status }}</span>
                            </header>
                            <div class="muted small">Praticien: {{ $plan->practitioner?->name ?: '-' }}</div>
                            <div class="money-row">
                                <span>Total: {{ number_format((float) $plan->total_estimated_cost, 2, ',', ' ') }} MAD</span>
                                <span>Paye: {{ number_format((float) $plan->paid_amount, 2, ',', ' ') }} MAD</span>
                                <span>Reste: {{ number_format($remain, 2, ',', ' ') }} MAD</span>
                            </div>
                            <div class="progress mt-2" style="height:8px">
                                <div class="progress-bar" style="width: {{ $progress }}%"></div>
                            </div>
                            <div class="small text-secondary mt-1">{{ $progress }}%</div>
                            <ul class="phase-list">
                                @foreach(($plan->phases ?? []) as $phase)
                                    <li>
                                        <span>{{ $phase['name'] ?? '-' }}</span>
                                        <span class="badge bg-azure-lt">{{ $phase['status'] ?? 'planned' }}</span>
                                    </li>
                                @endforeach
                            </ul>
                        </article>
            ...
            <section id="advanced-quote" class="card" data-care-tab-panel="documents">
                ...
                <div class="row g-3">
                    <div class="col-lg-6">
                        ...
                        <form method="POST" action="{{ route('care.module3.quote.create', ['plan' => $treatmentPlans->first()->id ?? 0]) }}" id="quoteCreateForm">
                            @csrf
                            <div class="row g-2">
                                <div class="col-md-6">
                                ...
                                <div class="col-12">
                                    ...
                                    <select class="form-select" name="plan_selector" onchange="this.form.action='{{ url('/care/module-3/treatment-plan') }}/'+this.value+'/quote';">
                                        @foreach($treatmentPlans as $p)
                                            <option value="{{ $p->id }}">{{ $p->name }} - {{ number_format((float) $p->total_estimated_cost, 2, ',', ' ') }} MAD</option>
                                        @endforeach
                                    </select>
                                </div>
                    ...
                    <div class="col-lg-6">
                        ...
                        @if(($treatmentQuotes->first()))
                            @php
                                $latestQuote = $treatmentQuotes->first();
                            @endphp
                            <form method="POST" action="{{ route('care.module3.quote.sign', ['quote' => $latestQuote->id]) }}">
                                @csrf
                                <div class="mb-2 small">Devis cible: <strong>{{ $latestQuote->quote_number }}</strong></div>
                                ...
                                <div class="d-flex gap-2 mt-2">
                                    ...
                                    <a class="btn btn-outline-primary" href="{{ route('care.module3.quote.pdf', ['quote' => $latestQuote->id]) }}" target="_blank">Voir PDF</a>
                                </div>
                            </form>
                        @else
                ...
                <div class="mt-3">
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm">
                            ...
                            @forelse($treatmentQuotes as $q)
                                <tr>
                                    <td>{{ $q->quote_number }}</td>
                                    <td>{{ optional($q->quote_date)->format('d/m/Y') }}</td>
                                    <td>{{ number_format((float) $q->subtotal, 2, ',', ' ') }} MAD</td>
                                    <td>{{ number_format((float) $q->insurance_amount + (float) $q->mutual_amount, 2, ',', ' ') }} MAD</td>
                                    <td>{{ number_format((float) $q->patient_amount, 2, ',', ' ') }} MAD</td>
                                    <td><span class="badge bg-indigo-lt">{{ $q->status }}</span></td>
                                    <td><a class="btn btn-sm btn-outline-primary" href="{{ route('care.module3.quote.pdf', ['quote' => $q->id]) }}" target="_blank">PDF</a></td>
                                </tr>
                            @empty
    ...
    <div class="section-head d-flex flex-wrap justify-content-between align-items-center gap-2">
        ...
        <div class="d-flex flex-wrap gap-2 align-items-center">
            <span class="badge bg-success-lt rx-head-badge">
                <i class="ti ti-shield-check me-1"></i>{{ $hasMajorRisk ? 'Securite active' : 'Securise' }}
            </span>
    ...
    <div class="rx-split-view">
        <form method="POST" action="{{ route('care.module3.prescriptions.store', ['patientId' => $selectedPatientId]) }}" id="rxForm" class="rx-editor-panel">
            @csrf
            ...
            <div class="rx-security-banner {{ $hasMajorRisk ? 'is-risk' : 'is-safe' }}">
                <div class="rx-security-icon">
                    <i class="ti {{ $hasMajorRisk ? 'ti-alert-triangle' : 'ti-shield-check' }}"></i>
                </div>
                <div>
                    <div class="fw-bold">{{ $hasMajorRisk ? 'Controle de securite requis' : 'Controle de securite valide' }}</div>
                    @if($hasMajorRisk)
                        <div class="small">{{ $allergies->isNotEmpty() ? 'Allergies: '.$allergies->implode(', ') : 'Aucune allergie declaree' }}</div>
                        <div class="small">{{ $riskTags->isNotEmpty() ? 'Risques: '.$riskTags->implode(', ') : 'Aucun risque majeur' }}</div>
                    @else
        ...
        <aside class="rx-preview-panel">
            ...
            <div class="rx-paper-sheet" id="rxPaperSheet">
                <div class="rx-paper-header">
                    ...
                    <div class="rx-paper-meta">
                        <div><strong>MRN</strong> <span id="rxPaperMrn">{{ $selectedPatient?->medical_record_number ?? '-' }}</span></div>
                        <div><strong>Date</strong> <span id="rxPaperDate">{{ now()->format('d/m/Y H:i') }}</span></div>
                    </div>
                </div>
                ...
                <div class="rx-paper-patient">
                    <div>
                        ...
                        <div class="rx-paper-name" id="rxPaperPatientName">{{ $selectedPatient?->full_name ?? 'Patient non selectionne' }}</div>
                    </div>
                    <div class="rx-paper-patient-meta">
                        <span id="rxPaperPatientAge">{{ $selectedPatient?->age ?? '-' }} ans</span>
                        <span id="rxPaperPatientInitials">{{ $selectedPatient ? strtoupper(substr($selectedPatient->first_name, 0, 1) . substr($selectedPatient->last_name, 0, 1)) : '--' }}</span>
                    </div>
                </div>
            ...
            <div class="rx-history-shell">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <span class="badge bg-azure-lt">{{ count($prescriptions) }}</span>
                </div>
                <div class="table-responsive">
                    <table class="table table-sm align-middle rx-history-table">
                        ...
                        <tbody>
                            @forelse($prescriptions as $rx)
                                <tr>
                                    <td>
                                        <div class="fw-bold">{{ $rx->prescription_number }}</div>
                                        <div class="small text-secondary">{{ optional($rx->issued_at)->format('d/m/Y H:i') }}</div>
                                    </td>
                                    <td class="text-center">
                                        <span class="badge {{ $rx->status === 'issued' ? 'bg-success-lt text-success' : 'bg-azure-lt text-azure' }}">{{ $rx->status }}</span>
                                    </td>
                                    <td class="text-end">
                                        <div class="btn-list justify-content-end flex-nowrap">
                                            <a href="{{ route('care.module3.prescriptions.pdf', ['prescription' => $rx->id]) }}" target="_blank" class="btn btn-icon btn-outline-primary btn-sm" title="PDF">
                                                <i class="ti ti-file-text"></i>
                                            </a>
                                            <a href="{{ route('care.module3.prescriptions.verify', ['token' => $rx->qr_token]) }}" target="_blank" class="btn btn-icon btn-outline-secondary btn-sm" title="Verifier">
                                                <i class="ti ti-qrcode"></i>
                                            </a>
                                            <form method="POST" action="{{ route('care.module3.prescriptions.send-email', ['prescription' => $rx->id]) }}" class="d-inline" title="Envoyer par email">
                                                @csrf
                                                <input type="hidden" name="email" value="{{ $selectedPatient?->email }}">
...
<section id="parodontal" class="card paro-card" data-care-tab-panel="clinical">
                ...
                <input type="hidden" id="paroRecordedOn" value="{{ now()->toDateString() }}">
                <div class="mt-3">
                    <h4 class="mb-2">Historique rÃ©cent</h4>
                    <div class="paro-history" id="paroHistoryList">
                        @forelse($periodontalHistorySeed as $chart)
                            <article class="paro-history-item">
                                <strong>{{ $chart['recorded_on'] }}</strong>
                                <div class="muted small">{{ $chart['summary'] ?: 'Sans synthese' }}</div>
                            </article>
                        @empty
            ...
            <section id="ortho-photos" class="card" data-care-tab-panel="documents">
                ...
                <form method="POST" action="{{ route('care.module3.ortho-photos.store', ['patientId' => $selectedPatientId]) }}" enctype="multipart/form-data" class="row g-2">
                    @csrf
                ...
                <div class="imaging-cards mt-3">
                    @forelse($orthoPhotoSets as $set)
                        <article class="imaging-card">
                            <div><strong>{{ $set->label }}</strong><div class="muted small">{{ optional($set->captured_on)->format('d/m/Y') }}</div></div>
                            <div class="imaging-actions d-flex gap-1">
                                @if($set->before_image_path)<button type="button" class="btn btn-sm btn-outline-primary btn-preview" data-src="{{ asset($set->before_image_path) }}">Avant</button>@endif
                                @if($set->after_image_path)<button type="button" class="btn btn-sm btn-outline-success btn-preview" data-src="{{ asset($set->after_image_path) }}">Apres</button>@endif
                            </div>
                        </article>
                    @empty
            ...
            <section id="legal-docs" class="card" data-care-tab-panel="documents">
                ...
                <div class="row g-3">
                    <div class="col-lg-6">
                        <form method="POST" action="{{ route('care.module3.legal.store', ['patientId' => $selectedPatientId]) }}" enctype="multipart/form-data" class="row g-2">
                            @csrf
                    ...
                    <div class="col-lg-6">
                        <form method="POST" action="{{ route('care.module3.questionnaire.store', ['patientId' => $selectedPatientId]) }}" class="row g-2">
                            @csrf
                ...
                <div class="mt-3">
                    @foreach($healthQuestionnaires as $qst)
                        <div class="mb-2 p-2 border rounded {{ $qst->has_critical_risk ? 'border-danger' : 'border-secondary' }}">
                            <strong>{{ optional($qst->filled_on)->format('d/m/Y') }}</strong>
                            @if($qst->has_critical_risk)<span class="badge bg-red-lt">Risque critique</span>@endif
                            <div class="muted small">{{ implode(', ', $qst->risk_tags ?? []) }}</div>
                        </div>
                    @endforeach
                </div>
            </section>
                        ...
                        <div class="questionnaire-launch-actions">
                            <select class="form-select form-select-sm questionnaire-launch-select" id="questionnaireQuickTemplateSelect">
                                @forelse($questionnaires as $template)
                                    <option value="{{ $template->id }}">{{ $template->name }} ({{ count($template->field_schema ?? []) }} champs)</option>
                                @empty
                        ...
                        <article class="questionnaire-history-card">
                            <div>
                                <div class="d-flex justify-content-between gap-2 flex-wrap">
                                    <strong>{{ $response->questionnaire?->name ?? 'Questionnaire' }}</strong>
                                    <span class="meta">{{ optional($response->answered_at)->format('d/m/Y H:i') }}</span>
                                </div>
                                <div class="meta">Consultation: {{ $response->consultation?->consultation_reason ?: '-' }} | Praticien: {{ $response->practitioner?->name ?: '-' }}</div>
                                <div class="small mt-1">{{ collect($response->answers ?? [])->take(3)->map(fn ($value, $key) => $key.': '.(is_array($value) ? json_encode($value) : (is_bool($value) ? ($value ? 'Oui' : 'Non') : $value)))->implode(' | ') }}</div>
                                @if($response->notes)
                                    <div class="small text-secondary mt-1">Notes: {{ $response->notes }}</div>
                                @endif
                                @if(collect($response->answers ?? [])->count() > 3)
                                    <details class="small mt-1">
                                        <summary>Voir toutes les reponses</summary>
                                        <div class="mt-1">{{ collect($response->answers ?? [])->map(fn ($value, $key) => $key.': '.(is_array($value) ? json_encode($value) : (is_bool($value) ? ($value ? 'Oui' : 'Non') : $value)))->implode(' | ') }}</div>
                                    </details>
                                @endif
                            </div>
                        </article>
            ...
            <section id="ai-diagnostic" class="card" data-care-tab-panel="clinical">
                ...
                <form method="POST" action="{{ route('care.module3.ai-analysis.store', ['patientId' => $selectedPatientId]) }}" class="row g-2">
                    @csrf
                ...
                <form method="POST" action="{{ route('care.module3.recalls.generate', ['patientId' => $selectedPatientId]) }}" class="mt-2">
                    @csrf
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-sm">
                        ...
                        @forelse($aiAnalyses as $analysis)
                            <tr>
                                <td>{{ optional($analysis->requested_at)->format('d/m H:i') }}</td>
                                <td>{{ $analysis->analysis_type }}</td>
                                <td>{{ $analysis->provider }}</td>
                                <td>{{ $analysis->confidence !== null ? $analysis->confidence.'%' : '-' }}</td>
                                <td><span class="badge bg-azure-lt">{{ $analysis->status }}</span></td>
                            </tr>
                        @empty
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-sm">
                        ...
                        <tbody>
                        @forelse($patientRecalls as $recall)
                            <tr>
                                <td>{{ $recall->reason }}</td>
                                <td>{{ optional($recall->due_date)->format('d/m/Y') }}</td>
                                <td><span class="badge bg-azure-lt">{{ $recall->status }}</span></td>
                                <td>{{ optional($recall->last_notified_at)->format('d/m/Y') ?: '-' }}</td>
                            </tr>
                        @empty
...
(() => {
    ...
    const initialTab = getTabFromUrl() || window.localStorage.getItem(storageKey) || '{{ $currentTab }}' || 'overview';
...
(() => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            closeSlideover();
            window.location.assign(data.redirect_url || response.url || '{{ route('care.module3.index') }}');
        } catch (_error) {
    ...
    if (selectedPatientId > 0) {
        const historyEndpoint = @json(url('/care/module-3/patients/'.$selectedPatientId.'/history'));
    ...
    patientHistoryForm?.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            const historyItemsStoreBase = @json(url('/care/module-3/patients'));
...
@include('modules.partials.questionnaire-modern')
<script>
(() => {
    const questionnaireTemplates = @json($questionnaireTemplatesPayload);
    ...
    const defaultConsultationId = @json(optional($consultations->first())->id);
    const responseStoreBase = @json(url('/care/module-3/patients/'.$selectedPatientId.'/questionnaires'));
...
function getCareSelectedPatientId() {
    ...
    return {{ (int) ($selectedPatientId ?? 0) }};
}

const careProcedurePatientsBaseUrl = @json(url('/care/module-3/patients'));
...
(() => {
    ...
    const data = @json($odontogramTeethStatus);
    const annotations = {};
...
(() => {
    ...
    const patientPreview = @json($rxPatientPreview);
    const catalog = @json($rxCatalogPayload);
    const templates = @json($rxTemplatePayload);
    const allergies = @json(($allergies ?? collect())->map(fn($v) => mb_strtolower((string) $v))->values());
    const riskTags = @json(($riskTags ?? collect())->map(fn($v) => mb_strtolower((string) $v))->values());
    const historyTags = @json(collect($selectedPatient?->medical_history ?? [])->map(fn($v) => mb_strtolower((string) $v))->values());
...
(() => {
    ...
    const selectedPatientId = {!! (int) ($selectedPatientId ?? 0) !!};
    const periodontalStoreBase = "{!! url('/care/module-3/patients') !!}";
    const saveUrl = selectedPatientId ? `${periodontalStoreBase}/${selectedPatientId}/periodontal-chart` : '';
    const initialSeed = @json($periodontalSeed ?? []);
    const initialHistory = @json($periodontalHistorySeed ?? []);
    ...
    const odontogramStatus = @json($odontogramTeethStatus ?? []);
...
@push('scripts')
<script src="{{ asset('js/inline-history-edit.js') }}?v={{ @filemtime(public_path('js/inline-history-edit.js')) ?: time() }}"></script>
<script src="{{ asset('js/hybrid-store.js') }}?v={{ @filemtime(public_path('js/hybrid-store.js')) ?: time() }}" defer></script>
@endpush

@if(in_array($currentSpecialtyCode, ['GYNECO', 'OMNI']))
@push('scripts')
@include('gynecology::partials.gynecology-scripts')
@endpush
@endif

@if(in_array($currentSpecialtyCode, ['PEDIA', 'OMNI']))
@push('scripts')
@include('pediatrics::partials.pediatrics-scripts')
@endpush
@endif

@if(in_array($currentSpecialtyCode, ['BURNS', 'OMNI']))
@push('scripts')
@include('burns::partials.burns-scripts')
@endpush
@endif

@if(in_array($currentSpecialtyCode, ['REHAB', 'OMNI']))
@push('scripts')
@include('rehab::partials.rehab-scripts')
@endpush
@endif

@if(in_array($currentSpecialtyCode, ['INTMED', 'OMNI']))
@push('scripts')
@include('internal-medicine::partials.internal-medicine-scripts')
@endpush
@endif
