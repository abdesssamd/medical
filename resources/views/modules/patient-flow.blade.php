...
@section('title', 'Module 2 - Agenda et Flux Patient')
@section('page_pretitle', 'Module 2')
@section('page_title', 'Agenda Intelligent et Flux Patient')
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" name="professional_id">
                    ...
                    @foreach($professionals as $pro)
                        <option value="{{ $pro->id }}" @selected($selectedProfessionalId === $pro->id)>{{ $pro->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-2">
...
function patientFlowApp() {
    return {
        csrf: '{{ csrf_token() }}',
        dateValue: @json($date),
        professionalId: @json($selectedProfessionalId),
        viewMode: 'table',
        items: @json($board['items']),
        scheduleRows: @json($scheduleGrid['rows'] ?? []),
        dragItem: null,
        kpi: { total: 0, booked: 0, inCare: 0, completed: 0 },
        spark: { total: '', booked: '', inCare: '', completed: '' },
        coord: { patient_id: '{{ $patients->first()?->id }}', from_date: @json($date), to_date: @json($date) },
