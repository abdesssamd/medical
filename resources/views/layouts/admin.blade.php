...
<head>
    ...
    <title>@yield('title', 'MediOffice') | Workspace</title>
...
<body class="app-layout @yield('body_class')">
...
<aside class="sidebar-modern" id="sidebar">
    ...
    <div class="role-badge">
        <div class="role-avatar">{{ substr($roleLabel, 0, 1) }}</div>
        <div class="role-text">{{ $roleLabel }}</div>
    </div>
    ...
    <ul class="nav-menu">
        @foreach($menu as $item)
            @if(isset($item['submenu']))
                ...
                @endphp
                <li class="nav-item">
                    <a class="nav-link-modern {{ $isSubmenuActive ? 'active submenu-open' : '' }}" href="#" onclick="toggleSubmenu(event, this)">
                        <i class="{{ $item['icon'] }}"></i>
                        <span style="flex:1">{{ $item['label'] }}</span>
                        <i class="ti ti-chevron-down submenu-icon"></i>
                    </a>
                    <ul class="submenu-list {{ $isSubmenuActive ? 'show' : '' }}">
                        @foreach($item['submenu'] as $subItem)
                            @if(isset($subItem['submenu']))
                                ...
                                <li>
                                    ...
                                    <ul class="submenu-list nested-submenu {{ $isNestedActive ? 'show' : '' }}">
                                        @foreach($subItem['submenu'] as $nestedItem)
                                            @php $isSubActive = request()->routeIs($nestedItem['active'] ?? ''); @endphp
                                            <li>
                                                <a href="{{ Route::has($nestedItem['route'] ?? '') ? route($nestedItem['route'], $nestedItem['params'] ?? []) : '#' }}" class="submenu-link {{ $isSubActive ? 'active' : '' }}">
                                                    {{ $nestedItem['label'] }}
                                                </a>
                                            </li>
                                        @endforeach
                            ...
                            @else
                                ...
                                <li>
                                    <a href="{{ Route::has($subItem['route'] ?? '') ? route($subItem['route'], $subItem['params'] ?? []) : '#' }}" class="submenu-link {{ $isSubActive ? 'active' : '' }}">
                                        {{ $subItem['label'] }}
                                    </a>
                                </li>
                            @endif
            ...
            @else
                ...
                <li class="nav-item">
                    <a class="nav-link-modern {{ $isActive ? 'active' : '' }}" href="{{ Route::has($item['route'] ?? '') ? route($item['route'], $item['params'] ?? []) : '#' }}">
                        <i class="{{ $item['icon'] }}"></i>
                        <span style="flex:1">{{ $item['label'] }}</span>
                        @if(($item['route'] ?? '') === 'care.module2.index')
        ...
        <div class="header-inner">
            <div style="display: flex; align-items: center;">
                ...
                <div class="page-title-section">
                    <div class="page-pretitle">@yield('page_pretitle', $roleLabel)</div>
                    <h1 class="page-title">@yield('page_title', 'Aperçu')</h1>
            ...
            <div class="user-section">
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-custom dropdown-menu-end">
                        <a href="{{ $homeRoute }}" class="dropdown-item-custom">
